...
export default function Stages({ Selection, assessmentStages }) {
  const [selectedLocation, setSelectedLocation] = useState("Written Exam");
  ...
  const icons = [
    <svg
      ...
      className="absolute -top-4 left-[-3%] lg:-top-3 md:left-[-15%] lg:left-[2%] "
    >
      <path
    ...
    <svg
      ...
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className="absolute -top-6 md:-top-3 lg:-top-6 left-[5%] md:left-[-30%] lg:left-[15%]"
    >
      <path
    ...
    <svg
      ...
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className="absolute -top-4 right-[20%] md:-top-4 md:right-[10%] -top-6"
    >
      <path
    ...
    <svg
      ...
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className="absolute -top-4 right-[20%] md:-top-4 md:right-[15%] -top-6"
    >
      <path
        d="M2 20C2 17 5 11 2 2M9.5 23.5C13.8333 19.3333 22.7 9.2 23.5 2M12.5 30.5C15.1667 30.5 22.3 29.1 29.5 23.5"
        stroke="#FFFDFC"
  ...
  const schools = [
    {
      ...
      location: "Florida",
      infoData: [
        { label: "Duration", text: "30–60 minutes" },
        {
          label: "Tip",
          text: "Be honest, stay calm, and answer naturally—there are no right or wrong responses.",
        },
      ],
    },
    {
      id: 2,
      location: "Texas",
      infoData: [{ label: "Duration", text: "30–60 minutes" }],
    },
    {
      id: 3,
      location: "kjvdi",
  ...
  const locations = [
    ...new Set(assessmentStages.map((school) => school.location)),
  ];
  const filteredSchools = assessmentStages.filter(
    (school) => school.location === selectedLocation
  );
  return (
    <div>
      <section
        className="w-full py-16 px-4 mt-6 bg-cover bg-center relative rounded-t-2xl"
        style={{
          backgroundImage: `url(${bgImage.src})`,
        }}
      >
        <div className="max-w-[90%] mx-auto grid gap-8 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4">
          {Selection.map(({ title, description }, index) => (
            <div
              key={index}
            ...
            >
              ...
              <div className="relative mb-4">
                {icons[index] || null}{" "}
                {/* Use icon based on index or default to null */}
                <h3 className="text-[24px] md:text-[32px] font-bold relative z-10 text-[#FCFCFD]">
                  {title}
                </h3>
              </div>

              <p className="text-[20px] text-[#F9FAFB]">{description}</p>
            </div>
          ))}
        </div>
      </section>
      {/* Background Section */}
      <div className="relative bg-[#2353AB]">
        ...
        <div className="rounded-lg mx-auto relative z-10">
          <div className="grid grid-row-6 gap-8 mx-auto w-[98%] lg:w-[70%]">
            {filteredSchools.map((school) => (
              <div
                key={school.id}
                className="py-2 lg:p-6 rounded-lg mt-[-20%] md:mt-[-5%]"
              >
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-8 justify-items-center bg-blue-50 px-12 py-8 rounded-[16px]">
                  {locations.map((location, index) => (
                    ...
                    >
                      <div className="text-center">
                        <p className="mt-1 text-center">{location}</p>
                      </div>
                    </button>
                  ))}
                </div>
                <div>
                  <p className="text-white text-[24px] md:text-[42px] text-center font-bold">
                    Personality Assessment
                  </p>
                  <p className="text-white text-center text-[16px] md:text-[24px]">
                    Conducted by professionals, this test evaluates your
                    personality traits to determine suitability for the program.
                  </p>
                </div>
                {/* Partner Flying Schools Section */}
                {filteredSchools.map((school) => (
                  <div key={school.id} className="py-4 lg:p-6 rounded-lg">
                    {/* Info Section */}
                    <div className="bg-[#1F56A3] p-6 flex flex-col gap-4">
                      {school.infoData.map((info, index) => (
                        <div
                          key={index}
                          className="bg-[#E9EFF8] rounded-[12px] flex flex-col md:flex-row text-left md:justify-between items-start md:items-center px-4 py-4"
                        >
                          {/* Left Label */}
                          <span className="bg-[#1F56A3] rounded-[12px] text-white text-[12px] md:text-[16px] px-3 py-1 font-semibold">
                            {info.label}
                          </span>

                          {/* Right Content */}
                          <p className="text-[#1F56A3] text-[16px] md:text-[24px] font-semibold text-left md:text-right max-w-[80%]">
                            {info.text}
                          </p>
                        </div>
                      ))}
